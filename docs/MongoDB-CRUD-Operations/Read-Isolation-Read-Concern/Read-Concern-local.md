...
具有读取关注点的查询`local`从实例返回数据，但不保证数据已写入大多数副本集成员(即：可能会回滚)。
...
* 在 Write<sub>0</sub>之后没有发生其他写操作。
  ...
  ![对三个成员副本集的写操作的时间轴。](https://docs.mongodb.com/manual/_images/read-concern-write-timeline.svg)
...
| 阅读目标              | Time `T`            | 数据状态                        |
| --------------------- | ------------------- | ------------------------------- |
| 主                    | 在t<sub>0</sub>之后 | 数据反映了 Write<sub>0</sub>    |
